...
describe('resolveCanvasCursor', () => {
  it('prioritizes panning and drag states', () => {
    ...
    expect(resolveCanvasCursor({ ...baseInput, dragState: { type: 'move' } as DragState })).toBe('grabbing');
    expect(resolveCanvasCursor({ ...baseInput, dragState: { type: 'crop' } as DragState })).toBe('crosshair');
  });
