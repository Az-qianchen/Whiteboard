...
const MultiSelectionControls: React.FC<{ paths: AnyPath[], scale: number }> = React.memo(({ paths, scale }) => {
    ...
    return (
        <g className="pointer-events-auto">
            ...
            <circle cx={rotationHandlePos.x} cy={rotationHandlePos.y} r={5 / scale} fill="var(--text-primary)" stroke={accent} strokeWidth={scaledStroke(1)} data-handle="rotate" style={{ cursor: 'grab' }} className="pointer-events-all" />
            {handles.map(({ pos, name, cursor }) => (<rect key={name} x={pos.x - halfHandleSize} y={pos.y - halfHandleSize} width={handleSize} height={handleSize} fill="var(--text-primary)" stroke={accent} strokeWidth={scaledStroke(1)} data-handle={name} style={{ cursor }} className="pointer-events-all" />))}
        </g>
    );
});
