...
export const ColorPicker: React.FC<ColorPickerProps> = ({ color, onChange, onInteractionStart, onInteractionEnd }) => {
  ...
  return (
    <div ref={pickerRef} className="w-64 bg-white dark:bg-[#4A5568] p-4 rounded-lg shadow-lg z-20">
      <div className="space-y-3">
        ...
        <div className="relative h-4 cursor-pointer" onPointerDown={createSliderHandler((p) => handleHslaChange({ h: p * 360 }))}>
            ...
            <div
                className="absolute w-4 h-4 -translate-y-1/2 -translate-x-1/2 top-1/2 rounded-full bg-white shadow-md ring-1 ring-gray-900/20"
        ...
        <div className="relative h-4 cursor-pointer" onPointerDown={createSliderHandler((p) => handleHslaChange({ s: p * 100 }), 20)}>
            ...
            <div
                className="absolute w-4 h-4 -translate-y-1/2 -translate-x-1/2 top-1/2 rounded-full shadow-md ring-1 ring-gray-900/20"
        ...
        <div className="relative h-4 cursor-pointer" onPointerDown={createSliderHandler((p) => handleHslaChange({ a: p }), 20)}>
            <div className="w-full h-2 rounded-lg absolute top-1/2 -translate-y-1/2 bg-slate-200 dark:bg-slate-600"/>
        ...
        <div className="flex items-center justify-between gap-2 pt-1">
           <div className="flex items-center bg-slate-100 dark:bg-slate-700 rounded-md h-9 px-2 w-[100px]">
             <input
               type="text"
               value={hexInput}
               ...
               className="w-full text-sm font-mono bg-transparent text-slate-800 dark:text-slate-200 focus:outline-none"
               aria-label="Hex color value"
             />
           </div>
           
           <div className="flex items-center bg-slate-100 dark:bg-slate-700 rounded-md h-9 px-2 w-[70px]">
               <input
                 type="number"
                 min="0"
                 max="100"
            ...
            {'EyeDropper' in window && (
                <button onClick={handleEyeDropper} className="w-9 h-9 flex-shrink-0 flex items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600" title="Pick color from screen">
      ...
      <div className="grid grid-cols-7 gap-2 mt-4">
        {PRESET_COLORS.map(c => (
          <button
            ...
            className="w-full aspect-square rounded-full ring-1 ring-inset ring-gray-300 dark:ring-slate-500"
         ...
         <button
            ...
            className="w-full aspect-square rounded-full ring-1 ring-inset ring-gray-300 dark:ring-slate-500 bg-white"
