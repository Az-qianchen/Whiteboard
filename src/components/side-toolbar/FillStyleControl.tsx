...
export const FillStyleControl: React.FC<FillStyleControlProps> = React.memo(({ fillStyle, setFillStyle }) => (
    <div className="flex flex-col items-center w-14 transition-opacity" title={`填充样式: ${FILL_STYLES.find(s => s.name === fillStyle)?.title}`}>
        <Popover className="relative">
            <Popover.Button className={`${BUTTON_SIZE_SM} p-1.5 rounded-lg flex items-center justify-center transition-colors text-[var(--text-secondary)] hover:bg-[var(--ui-element-bg-hover)] ring-1 ring-inset ring-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] disabled:cursor-not-allowed`} title={`填充样式: ${FILL_STYLES.find(s => s.name === fillStyle)?.title}`} aria-label="选择填充样式">
                    ...
                    {({ close }) => (
                        <div className="grid grid-cols-1 gap-1">
                            {FILL_STYLES.map(({ name, title, icon }) => (
                                <button key={name} onClick={() => { setFillStyle(name); close(); }} className={`w-full flex items-center gap-3 p-2 rounded-md text-left text-sm transition-colors ${fillStyle === name ? 'bg-[var(--accent-bg)] text-[var(--accent-primary)]' : 'hover:bg-[var(--ui-element-bg-hover)] text-[var(--text-primary)]'}`}>
