...
export const DashControl: React.FC<DashControlProps> = React.memo((props) => {
    ...
    const dashTitle = t('sideToolbar.dash.title');
    const enableLabel = t('sideToolbar.dash.enable');
    const lengthLabel = t('sideToolbar.dash.length');
    const gapLabel = t('sideToolbar.dash.gap');
    const scrollHint = t('sideToolbar.dash.scrollHint');

    return (
        <div className={`flex flex-col items-center w-14 transition-opacity ${strokeWidth === 0 ? 'opacity-50' : ''}`} title={dashTitle}>
                ...
                <Transition as={Fragment} enter="transition ease-out duration-200" enterFrom="opacity-0 translate-y-1" enterTo="opacity-100 translate-y-0" leave="transition ease-in duration-150" leaveFrom="opacity-100 translate-y-0" leaveTo="opacity-0 translate-y-1">
                    <Popover.Panel className="absolute bottom-0 mb-0 right-full mr-2 w-48 bg-[var(--ui-popover-bg)] backdrop-blur-lg rounded-xl shadow-lg border border-[var(--ui-panel-border)] z-20 p-4">
                        <div className={PANEL_CLASSES.section}>
                            <SwitchControl label={enableLabel} enabled={dashGap.isDashed} setEnabled={dashGap.handleToggleDashed} />
                            <div className={`space-y-3 transition-opacity ${!dashGap.isDashed ? 'opacity-50 pointer-events-none' : ''}`}>
                                <div className="grid grid-cols-2 items-center gap-2">
                                    <label htmlFor="dash-length-input" className={`${PANEL_CLASSES.label} text-[var(--text-primary)]`}>{lengthLabel}</label>
                                    <div className={`${PANEL_CLASSES.inputWrapper} cursor-ns-resize`} onWheel={dashGap.handleDashWheel} title={scrollHint}>
                                        <input
                                ...
                                <div className="grid grid-cols-2 items-center gap-2">
                                    <label htmlFor="dash-gap-input" className={`${PANEL_CLASSES.label} text-[var(--text-primary)]`}>{gapLabel}</label>
                                    <div className={`${PANEL_CLASSES.inputWrapper} cursor-ns-resize`} onWheel={dashGap.handleGapWheel} title={scrollHint}>
                                        <input
                                            id="dash-gap-input"
                                            type="text"
                                            inputMode="numeric"
