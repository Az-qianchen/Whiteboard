...
export const CropToolbar: React.FC<CropToolbarProps> = ({
  ...
  const handleFeatherRadiusChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const clampedValue = Math.min(200, Math.max(0, parsedValue));
    ...
    >
      ...
      {cropTool === 'magic-wand' && (
        <div className="flex flex-wrap items-center gap-3">
          ...
          <label className="flex items-center gap-2 text-sm text-[var(--text-secondary)]" htmlFor="magic-wand-feather">
            {t('featherRadius')}
            <div className={`${PANEL_CLASSES.inputWrapper} w-16`}>
              <input
                id="magic-wand-feather"
                type="number"
                min={0}
                max={200}
                step={1}
