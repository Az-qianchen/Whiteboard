...
export const useAppStore = () => {
  ...
  const handleCropManualPointerMove = useCallback((point: Point) => {
    ...
    const isNearStart = Boolean(
      ...
      dist(point, firstPoint) <= closeThreshold
    );

    cropManualDraftRef.current = {
      ...draft,
      previewPoint: point,
      closingHint: isNearStart ? 'near-start' : undefined,
    };

    setAppState(s => {
      const current = s.cropManualDraft;
      if (!current || current.mode !== 'polygon') return s;
      const samePreview = current.previewPoint?.x === point.x && current.previewPoint?.y === point.y;
      const nextHint = isNearStart ? 'near-start' : undefined;
      if (samePreview && current.closingHint === nextHint) return s;
      return { ...s, cropManualDraft: { ...current, previewPoint: point, closingHint: nextHint } };
    });
  }, [setAppState, viewTransform.viewTransform.scale]);
  ...
  const handleCropManualPointerUp = useCallback((_event?: React.PointerEvent<SVGSVGElement>) => {
    ...
    const baseDraft = { mode: 'polygon' as const, operation: draft.operation, points: draft.points };
    cropManualDraftRef.current = baseDraft;

    setAppState(s => {
      const current = s.cropManualDraft;
      if (!current || current.mode !== 'polygon') return s;
      if (!current.previewPoint && current.closingHint === undefined) return s;
      return { ...s, cropManualDraft: baseDraft };
    });
