  ...
  if (keepAspectRatio && oldWidth > 0 && oldHeight > 0) {
    ...
    if (isCorner) {
      if (Math.abs(newWidth) > Math.abs(newHeight) * targetRatio) {
        newHeight =
          (Math.abs(newWidth) / targetRatio) * Math.sign(newHeight || (handle.includes('bottom') ? 1 : -1));
      } else {
        newWidth =
          Math.abs(newHeight) * targetRatio * Math.sign(newWidth || (handle.includes('right') ? 1 : -1));
      }
    } else if (affectsX) {
