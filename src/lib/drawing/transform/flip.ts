...
export async function flipPath(path: AnyPath, center: Point, axis: 'horizontal' | 'vertical'): Promise<AnyPath> {
  ...
  switch (path.tool) {
    ...
    case 'text': {
      ...
      const newY = newShapeCenter.y - height / 2;
      const newRotation = -(rotation ?? 0);
      const flippedAlign = axis === 'horizontal'
        ? (textPath.textAlign === 'left' ? 'right' : textPath.textAlign === 'right' ? 'left' : textPath.textAlign)
        : textPath.textAlign;
      return { ...textPath, x: newX, y: newY, rotation: newRotation, textAlign: flippedAlign };
    }
    case 'frame':
    case 'rectangle':
    case 'ellipse':
