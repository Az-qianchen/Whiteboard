...
import type { Point, AnyPath, RectangleData, EllipseData, VectorPathData, BBox, ImageData, BrushPathData, PolygonData, ArcData, GroupData, TextData, FrameData } from '../types';
...
export function isPointHittingPath(point: Point, path: AnyPath, scale: number): boolean {
    ...
    switch (path.tool) {
        ...
        case 'rectangle':
        case 'text': {
            const { x, y, width, height, rotation } = path as RectangleData | ImageData | FrameData | TextData;
            let testPoint = point;
